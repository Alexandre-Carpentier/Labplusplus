...
void cSupplysim::set_configuration_struct(CURRENT_DEVICE_CONFIG_STRUCT config_struct)
{
    config_struct_ = config_struct;
    return;
}

int cSupplysim::launch_device()
{
    return 0;
}
...
{
    result.buffer[0] = 0.5 + (((double)(rand() % 10) + 15.0) / 100.0);
