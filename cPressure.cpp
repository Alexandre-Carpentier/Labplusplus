...
cPressure::cPressure(wxWindow* inst)
{
	...
	inst_->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cPressure::OnPressureEnableBtn, this, IDCDAQACTIVATE);
	...
	inst_->Bind(wxEVT_COMMAND_COMBOBOX_SELECTED, &cPressure::OnDaqAddrSelBtn, this, IDCADDR);
	...
	inst_->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cPressure::OnPreviousCliqued, this, IDCPREVIOUS);
	...
	inst_->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cPressure::OnNextCliqued, this, IDCNEXT);
	...
	for (int i = 0; i < max_chan_number; i++)
	{
		...
		inst_->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cPressure::OnChannelBtnNumberCliqued, this, IDCCHANBTN0 + i);
	...
	inst_->Bind(wxEVT_COMMAND_BUTTON_CLICKED, &cPressure::OnDaqChanEnableBtn, this, IDCCHECKCHAN);
	...
	inst_->Bind(wxEVT_TEXT, &cPressure::OnDaqChanNameModified, this, IDCCHANNAME);
	...
	inst_->Bind(wxEVT_TEXT, &cPressure::OnDaqChanPhysicalAddressNameModified, this, IDCCHANADDR);
	...
	inst_->Bind(wxEVT_TEXT, &cPressure::OnDaqChanTypeModified, this, IDCCHANTYPE);
...
void cPressure::show_right_panel(bool show)
{
	config_rightpanel_->Hide();
}

void cPressure::show_channel(const int index, bool show)
{
	//flexchansizer[index]->Show(show);
	// TODO load data into ctrls
}

void cPressure::show_channel(bool show)
{
...
void cPressure::show_next_channel()
{
...
void cPressure::OnNextCliqued(wxCommandEvent& evt)
{
	show_next_channel();
...
void cPressure::show_previous_channel()
{
...
void cPressure::OnPreviousCliqued(wxCommandEvent& evt)
{
...
void cPressure::save_current_chan_config(int channel_index)
{
...
// happen each time previous or next is pressed
void cPressure::load_current_chan_config(int channel_index)
{
...
/*
void cPressure::EnableChanProperties()
{
...
void cPressure::OnDaqAddrSelBtn(wxCommandEvent& evt)
{
...
void cPressure::DaqChanAllOn(bool enable)
{
...
void cPressure::OnDaqChanNameModified(wxCommandEvent& evt)
{
	std::cout << "cObjectmanager->getInstance()\n";
	cObjectmanager* object_manager = object_manager->getInstance();
	cPlot* m_plot = object_manager->get_plot();
...
void cPressure::OnDaqChanPhysicalAddressNameModified(wxCommandEvent& evt)
{
	std::cout << "cObjectmanager->getInstance()\n";
	cObjectmanager* object_manager = object_manager->getInstance();
	cPlot* m_plot = object_manager->get_plot();
...
void cPressure::OnDaqChanTypeModified(wxCommandEvent& evt)
{
	// Save current device and channels
	save_current_device_config(label.channel_index);
	save_current_chan_config(label.channel_index);
...
void cPressure::OnDaqChanEnableBtn(wxCommandEvent& evt)
{
	// Enable/disable controls
...
void cPressure::OnChannelBtnNumberCliqued(wxCommandEvent& evt)
{
...
void cPressure::EnableChannelItems(bool isDisplayed)
{
	if (!isDisplayed)
	{
		checkchan->SetBackgroundColour(wxColor(250, 120, 120)); // RED
...
void cPressure::show_voltage_param(bool show)
{
...
void cPressure::show_tc_param(bool show)
{
...
void cPressure::reload_current_channel_type()
{
	int ires = meas_type_ctrl->GetSelection();
	switch (ires)
	{
...
size_t cPressure::get_channel_index()
{
	return label.channel_index;
}

int cPressure::GetChannelNumber()
{
	return label.chan_number;
}

std::vector<bool> cPressure::GetChannelEnabledVector()
{
	return label.channel_enabled;
}

CURRENT_DEVICE_CONFIG_STRUCT cPressure::GetDaqConfigStruct()
{
	return config;
}
