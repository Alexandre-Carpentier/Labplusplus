...
cVoltage::cVoltage(wxWindow* inst, std::shared_ptr<cDeviceMonitor> devmon, cSignalTable* signal_table)
{
...
void cVoltage::EnableVoltageChannel(bool isDisplayed)
{
	...
	else
	{
		...
		*/
		...
		if (!signal_table_->sig_add(0, MEAS_TYPE::VOLTAGE_CONTROLER_INSTR, "2280S_A", instr_name, "Amp", wxColor(25, 200, 130)))
		{
			MessageBox(nullptr, L"Critical error at slot_register in cSignalTable, cannot register voltage signal.", L"[!] Critical failure.", S_OK);
		}
		if (!signal_table_->sig_add(1, MEAS_TYPE::VOLTAGE_CONTROLER_INSTR, "2280S_V", instr_name, "V", wxColor(200, 27, 27)))
		{
			MessageBox(nullptr, L"Critical error at slot_register in cSignalTable, cannot register voltage signal.", L"[!] Critical failure.", S_OK);
		}
